-- Stage B3, FUNCTIONS ONLY — apply this BEFORE the code that calls them reaches production.
-- ⚠️ Deliberately WITHOUT the three `revoke all on …` lines that close squads / user_prefs /
--    player_watchlist. This file only ADDS: nothing existing loses a permission, so it is safe to apply
--    ahead of the deploy and harmless if applied twice.
--
-- ⭐ **Stage B is SQL-first**, the opposite of Stage A. The new code CALLS these functions, so deploying it
--    first means every squad save and load 404s. That ordering has already cost one outage (2026-09-19,
--    the gate) — and Streamlit Cloud auto-deploys from master, so 'pushed' becomes 'live' without asking.
--
-- The revokes live in sql/stage_b3.sql and are run separately, once the code is confirmed deployed.
