-- Stage B, FUNCTIONS ONLY — the hotfix for a deployed app whose code expects these RPCs.
-- ⚠️ Deliberately WITHOUT `revoke all on public.beta_users from anon` — this file only ADDS.
-- Nothing existing changes, so it is safe to apply to production ahead of the full cutover.
-- ⭐ The ordering rule differs by stage: Stage A was code-first (a plain INSERT works against the old
--    policies); Stage B is SQL-FIRST, because the new code CANNOT work until these exist.
