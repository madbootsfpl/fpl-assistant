...
def _rpc(name: str):
    """`(url, key)` for a Postgres function over PostgREST — a sibling of the table endpoint (Stage B3).

    ⭐ The app asks by exact key and gets one row. It can no longer list handles, which is the difference
    between *"one request takes the table"* and *"one guess gets one row"*.
    """
    url, key = _config()
    if not (url and key):
        return None, None
...
def save_squad(handle: str, squad: dict) -> None:
    ...
    def _post():
        r = requests.post(url, json={"p_handle": h, "p_data": squad},
...
def load_squad(handle: str) -> dict | None:
    ...
    def _post():
        r = requests.post(url, json={"p_handle": h}, headers=_headers(key), timeout=_TIMEOUT)
        r.raise_for_status()
        return r

    return with_retry(_post, retries=1).json() or None


def exists(handle: str) -> bool:
    """True if a squad is already saved under `handle` (US-321). `False` when unconfigured, a bad handle, or
    nothing stored. Used to warn before a Save overwrites.

    ⭐ Returns a **boolean from the database**, not a row — the question is "new or overwrite?", and answering
    it used to require a select the caller could have widened."""
    url, key = _rpc("squad_exists")
    if not (url and key):
        return False
    h = clean_handle(handle)
    if not h:
...
def updated_at_by_handle(handles) -> dict:
    ...
    if not handles:
        return {}
    # ⭐ **Owner-only, so it needs a CREDENTIAL rather than a function** (Stage B3) — the same call as the
    # allow-list roster. A per-handle RPC would work but would be one round trip per tester on every Admin
    # render, which is exactly what this batched read exists to avoid.
    # ⚠️ Service-role: bypasses RLS, server-side only, never in a mobile client.
...
def delete_squad(handle: str) -> None:
    """Remove the squad saved under `handle`. Raises on a failed write; a no-op when unconfigured."""
    url, key = _rpc("delete_squad")
    if not (url and key):
        return
    h = clean_handle(handle)
    if not h:
