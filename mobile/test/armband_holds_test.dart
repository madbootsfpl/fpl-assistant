...
MyTeam sampleTeam() {
  final file = File('../spikes/018-flutter-read-slice/api-samples/my-team.json');
  return MyTeam.fromJson(jsonDecode(file.readAsStringSync()) as Map<String, dynamic>);
}
...
Draft asTheAppBuildsIt(MyTeam team, int managerId, {int? captainId, int? viceCaptainId}) {
  final base = Draft(
    managerId: managerId,
...
Draft asTheAppBuildsItAgain(Draft existing, MyTeam team, {int? captainId, int? viceCaptainId}) {
  final base = existing;
  final nextCaptain = captainId ?? base.captainId ?? team.captainId;
  final nextVice = viceCaptainId ?? base.viceCaptainId ?? team.viceCaptainId;
  final collides = nextVice == nextCaptain;
...
void main() {
  ...
  test('the armband survives a save and a reload', () async {
    ...
    final newCaptain = team.analysis.xi.firstWhere((p) => p.id != team.captainId).id;
    ...
    expect(back!.captainId, newCaptain, reason: 'the captain did not survive storage');
    ...
    expect(verdict, DraftStaleness.fresh,
    ...
    expect(shown.captainId, newCaptain, reason: 'the overlay did not reach the pitch');
  });
  ...
  test('promoting your vice to captain does not leave him wearing both', () {
    ...
    expect(vice, isNotNull, reason: 'the sample has no vice-captain to promote');

    final draft = asTheAppBuildsIt(team, 1, captainId: vice);
    expect(draft.captainId, vice);
    expect(draft.viceCaptainId, isNot(vice),
        reason: 'one player is wearing C and V at once — FPL would reject this squad');
  });
  ...
  test('changing your mind moves the armband', () {
    ...
    expect(twice.captainId, second, reason: 'the armband did not move to the second player');
  });

  test('promoting your vice AFTER a vice has been set clears the vice slot', () {
    ...
    expect(promoted.viceCaptainId, isNot(b),
  ...
  Draft asTheAppPlansASwap(Draft? existing, MyTeam team, int outId, int inId) => Draft.swap(
        existing: existing,
        managerId: 1,
        gameweek: team.gameweek ?? 0,
        basePlayerIds: team.fplPlayerIds,
  ...
  test('a transfer does not take your armband off — THE bug', () {
    ...
    expect(afterSwap.playerIds, contains(incoming), reason: 'the transfer itself must still happen');
    expect(afterSwap.captainId, captain, reason: 'the armband came off during a transfer');
  });
  ...
  test('transferring away your captain takes the armband with him', () {
    ...
    expect(afterSwap.captainId, isNot(captain),
  ...
  test('selling a bench player puts his replacement on the bench', () {
    ...
    expect(draft.benchIds, isNot(contains(out)), reason: 'the sold player is still on the bench');
    expect(draft.benchIds, contains(incoming), reason: 'his replacement never reached the bench');
  });
  ...
  test('the armband does not quietly reorder your bench', () {
    ...
    final team = sampleTeam();
    final draft = asTheAppBuildsIt(team, 1, captainId: team.analysis.xi.first.id);
    expect(draft.swaps, isEmpty, reason: 'an armband change produced transfers');
    expect(draft.benchIds.toSet(), team.analysis.bench.map((p) => p.id).toSet());
  });
}
