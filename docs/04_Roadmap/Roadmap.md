...
- 🚧 **Beta usage & experience analytics** — **foundation DONE + owner-verified (Sprint 136, ADR-100):** rows
  landing in Supabase, the app unaffected. Anonymous, opt-in (`FPL_ANALYTICS`), fail-silent usage + `perf` events →
  a Supabase `events` table (reuses the store, no new secret); `session_started`/`page_viewed` on all pages; the
  guardrail (never affects the app). **Sprint 137:** full event coverage + perf timers + a gated admin view.
  Deferred further: batching, a BI dashboard, cohorts.
