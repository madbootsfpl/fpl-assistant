...
| **Cold-start xMins forced to 1.0** (ADR-172 §⛔) | ⛔ **blocked on the backfill below**, then GW4-6 | ⚠️ **new, measured 2026-09-01.** A no-history player is modelled as a nailed 90-minute starter: **Mendy projects on 60 minutes a game, Yalcouyé on 38.** ADR-172 fixed the *rate* half of this pair; this is the *minutes* half, stacked on top, and it wants its own gate because fixing both at once would leave neither measurable |
...
| 🔄 **Run `python app.py history --backfill`** | ⛔ **It is blocking the xMins work.** `player_history` holds **GW1 only**, while the aggregate `players.minutes` already has two gameweeks (Sangaré: 75 in GW1, 165 total) — the per-GW backfill has not run since GW2 finished. Any in-season minutes share built today would come from **one gameweek** and contradict the aggregate every other surface reads. Design is ready and ADR-125's `minutes = 0` trap is already solved by `yet_to_play` (ADR-138) — the data is what is missing |
