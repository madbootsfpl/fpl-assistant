...
|---|---|---|
| `FORM_WEIGHT` · `SET_PIECE_WEIGHT` · `DEFCON_MAGNIFIER_WEIGHT` | **GW4**, real sitting **GW6** | criteria pre-registered in **GW1_RUNBOOK §B0** — four numeric bars, a stopping rule, and expected results recorded so a surprise is legible. ✅ **The harness itself was dry-run and repaired 2026-09-02** (§Pre-flight): it never printed the `n` criterion 1 is written in, and reported *"no clear signal"* for runs that evaluated nothing. Both fixed **before** there was a curve to look at. **2 of the 4 gameweeks needed** — GW3 is in flight |
...
| The **6** uncommitted files in your tree | **4 modified** — 3 sprint lesson docs + `spikes/015-soccerdata/compare_npxg.py`, kept out of every commit deliberately — **plus the 2 `.jpeg`s** they depend on. ⚠️ **Correction (2026-09-02): an earlier version of this row said the images were "mentioned nowhere". They are not stray.** Both are embedded in `Sprint61_Lessons_Learnt.md` as design references under *Notes (for Tony)* — *"Fixtures: could we display like attached?"* and *"MySquad: could we display like this…"*. They are untracked, so **git holds no copy and deleting them is unrecoverable**; they belong with the doc that uses them, whenever that doc is committed. The original claim was made from the filenames looking like camera output, without grepping for references. |
