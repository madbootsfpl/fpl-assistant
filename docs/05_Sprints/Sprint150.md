...
> EVE (A)), plus a **Total** when horizon >3. Reuses `by_gameweek` (already computed) — **display-only, no xP math**.
...
- **Horizon** 1/2/3/4/5/10 → `len(gameweeks)`; the **Total** column shows iff `>3`.
...
and the pitch hover popover (desktop) — Total when horizon >3, with no xP change and the suite green.

#### Success criteria
- [ ] **US-367 (the per-GW row + the panel card)** — extend `card_body`: when each `fixtures` item carries an `xp`,
      render a **per-GW row** (column = xP bold on top + FDR-tinted `OPP (H/A)` below) instead of the plain pills;
      add a `total_xp` param → a **Total** column (caller passes it only when horizon >3). **Backward-compatible:**
      no `xp` on fixtures → today's pills + Proj chip (the Players "Card" view unchanged). Build a shared
      `fixtures_by_id` in `render_my_squad` (`team_schedule` next-3 + `by_gameweek` xP per event) and wire the ⚙
      **panel card** (`render_player_card`). Tests: the panel card shows a per-GW xP + fixture; a Total appears at
      horizon 5, not at horizon ≤3.
...
│  photo   Team · POS · £8.0     │
│          Mbeumo                │
│   5.1     6.2     4.5    (12.7)│   ← per-GW row: xP (bold) …
│  HUL(A)  IPS(H)  EVE(A)  Total │   ← … over fixture (FDR-tinted); Total only when horizon>3
│  [flags: ownership · set-piece]│
└───────────────────────────────┘
```

- `card_body`: new `.plc-gwrow` of `.plc-gwcol` (xP over an FDR-tinted `OPP (H/A)`), + a `.total` col when `total_xp`
  given. Falls back to the current pills when fixtures carry no `xp`.
- `render_my_squad`: `fixtures_by_id[id] = [{opp,home,fdr,xp} …≤3]` via `team_schedule` + `by_gameweek_by_id`; pass to
  `render_pitch` (US-368) and to the panel card (US-367). `total_xp = xp_by_id[id]` when `len(gameweeks) > 3`.

**DoD:** tests (per-GW row in the panel card + Total rule; per-GW in the pitch markup) + a manual smoke (hover a shirt
